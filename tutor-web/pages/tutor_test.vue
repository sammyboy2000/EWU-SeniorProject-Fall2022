    ...
    <v-row>
      <v-col cols="8">
        <v-card v-show="userOption == 1">
          ...
          <v-card-text>
            <v-select v-model="selectedClass" :items="classData" label="Class" style="width: 25%; padding: 5px;" v-on:change="initializeQuestionData(selectedClass)"></v-select>
            <v-card-title v-if="selectedQuestion !== null">
              Selected Question on right to answer: {{ selectedQuestion.question1 }}
            </v-card-title>
...
export default class Tutor extends Vue {
  ...
  getAnsweredQuestionData() {
    this.$axios.get('/Questions/GetTutorAnsweredQuestions', {
      params: {
        tutorUsername: this.tutorUsername
      }
    }).then((response) => {
